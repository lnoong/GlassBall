...
class Detecter:
    def __init__(self, mvs_path):
        ...
        self.cam, stDeviceList = camera.creat_camera(deviceList, nConnectionNum, log=False)
        camera.open_device(self.cam)
        self.detector = YOLOv5Detector("best.onnx")
        cv2.waitKey(3000)
        camera.start_grab_and_get_data_size(self.cam)

    def __del__(self):
        camera.close_and_destroy_device(self.cam)
    ...
    def Detect(self, threshold):
        # image = camera.getOneFrame(self.cam)
        image = cv2.imread("D:\\AllProjects\\CProject\\opencv_project\\testpic\\pnp_6.bmp", 1)
        img = image
